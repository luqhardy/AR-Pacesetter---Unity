...
**ホスト側のリンク設定はコミットすること。さもないとCIは別物を検証している。**
Embed & Sign が開発者のローカルXcodeにしか無い状態だと、CIは
`canImport(UnityFramework)` が **false** の側をコンパイルします。
ビルドは通り、CIは緑になり、**フレームワーク有りの分岐は一度も触られません**。
`__mh_execute_header` のようなリンクエラーが、パイプラインが緑のまま実機へ届くのはこの経路です。
フレームワーク参照と Embed Frameworks フェーズを `pbxproj` へコミットすると、
リンク行に `-framework UnityFramework` が立って本物の分岐が検証されます。
**推測せず、ビルドログでこのフラグを確認してください。**

`Data` の Target Membership は同じ方法ではコミットできません — エクスポートのたびに
作り直される**生成物側**にあるためです。エクスポート後のスクリプトとして自動化します。

**エクスポートがソースと一致しているかも検査すること。** CIはダウンロードしてきた
Unity成果物に対してSwiftをリンクするだけなので、それがC#の変更より古い場合、
**古いフレームワークに対して緑になります** — 追いかけている修正がそもそも入っていません。
エクスポート元コミットと `HEAD` を比べ、`Assets/` `ProjectSettings/` `Packages/` に
差分があれば警告を出します。浅い `git fetch --depth=1 <sha>` で足ります —
`git diff` は2つのツリーさえあれば計算でき、繋がった履歴は不要だからです。
