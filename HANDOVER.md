...
- **§10の接地(±5cm)は未達: 立脚期の足が床へ約6cm沈む**(2026-09-24 判明)。以前のE2Eは足裏を1フレームで測っており、歩幅の位相次第で合格していた(-0.031m)。1歩幅ぶん毎フレーム採取して立脚期(最下点)で判定する形(`GroundContactMath`)に直したところ、足首ボーン推定で **-0.060m**、スキンメッシュの最下頂点で **最大-0.104m**(24フレーム中22フレームで床下)。足首→足裏の推定値の誤差ではなく実際のめり込み。原因はMixamo走行クリップの足がGroundSnapの原点平面より下がることと推定(未確認)。修正には立脚足基準のオフセットかフットIKが要り、エディタでの目視確認が必要。E2Eは§10の合否を `[E2E] INFO` として報告し、回帰(浮き +5cm超 / -8cmより深い沈み)のみを失敗とする
