...
開発者モードに `render.maxFrameMs`(走行中の最悪フレーム)・`render.longFrames`(約400ms以上)・`avatar.footPlanting`(メッシュ由来の足裏点か)を追加し、E2Eでキーの存在を検証。
検証: tools/verify.sh 全通過(dotnet test 355/355、Swift 24、E2E 210/210)

### 2026-09-25 (2) — 走行ログCSVの解析ツール(tools/analyze-run-log)

実地テストのT2(M2P p95 ≤ 20ms)を手計算させないため、`Log_*.csv` からM2Pのp50/p95/p99/最大・超過率とPASS/FAIL、100Hz記録の実レートと欠落、GPSの途絶(1.5秒以上)と距離(T4)、IMU・アバターのペースを出すツールを追加。複数ファイルは全体のp95も出し、FIELD_TEST_PLAN §3 の記入行をそのまま出力。`--json` 対応。
解析ロジック `RunLogAnalysis.cs` はユニットテスト15件。M2Pの基準値はゲーム本体の `MotionToPhotonMath` を共有。CIと `tools/verify.sh` で解析ツールのビルドも確認。
検証: tools/verify.sh 全通過(dotnet test 355/355、Swift 24、E2E 209/209)。E2Eの実ログと実機相当の合成ログで出力を確認
