...
// Unity side: use UnitySendMessage / NativeCallProxy pattern.
...
final class UnityBridge: NSObject, ObservableObject {
    ...
    // Called by Unity via UnitySendMessage("UnityBridge", "OnUnityMessage", jsonString)
    ...
    private func sendToUnity(object: String, method: String, payload: [String: Any]) {
        ...
        // In production with Unity as a Library:
        // UnityFramework.getInstance()?.sendMessageToGO(withName: object, functionName: method, message: json)
    ...
    /// Simulate Unity responses during development (remove in production)
